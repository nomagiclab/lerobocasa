...
- `lerobocasa/demo_style_mapping.json` (demo <-> style conversion)
...
Example:
  python3 lerobocasa/scripts/build_fixture_style_groups.py \\
    --fixtures-dir /Users/sepehrnasiriany/lerobocasa-dev/docs/_static/fixtures/sinks \\
    --fixture sink
"""
...
def _find_repo_root(start: Path) -> Path:
    """
    Walk upwards until we find lerobocasa/demo_style_mapping.json.
    """
    p = start.resolve()
    for cand in [p, *p.parents]:
        if (cand / "lerobocasa" / "demo_style_mapping.json").exists():
            return cand
    raise FileNotFoundError(
        f"Could not find repo root from {start}. Expected lerobocasa/demo_style_mapping.json somewhere above."
    )
...
def build_groups_for_fixture(
    *,
    ...
    demo_map_path = repo_root / "lerobocasa" / "demo_style_mapping.json"
