...
import numpy as np
import lerobocasa  # we need this to register environments  # noqa: F401
import robosuite
import sys
...
from lerobocasa.utils.env_utils import create_env
...
def create_robocasa_gym_env_class(env):
    class_name = f"{env}"
    id_name = f"lerobocasa/{class_name}"
    ...
    current_module = sys.modules["lerobocasa.wrappers.gym_wrapper"]
    setattr(current_module, class_name, env_class_type)
    register(
        id=id_name,  # Unique ID for the environment
        entry_point=f"lerobocasa.wrappers.gym_wrapper:{class_name}",  # Path to your environment class
    )
